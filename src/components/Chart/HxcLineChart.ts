      ...
      watch(
        ...
        (newValue) => {
          if (chart.value) {
            ...
            if (newValue.destroy) {
              chart.value.destroy(true);
              chart.value = (Highcharts as any).chart(
                chartRef.value,
                newValue,
                () => {
                  emit("rendered");
            ...
            // var length = chart.value.series.length;
            // if (length) {
            //   for (var j = 0; j < length; j++) {
            //     console.log(`'assdsd`, chart.value.series[j]);
            //     if (chart.value.series[j]) chart.value.series[j].remove(false);
            //   }
            // }

            // newValue.series.forEach((el: any) => {
            //   chart.value.addSeries(el, true);
            // });
            // chart.value.redraw(true);

            //chart.value.series = newValue.series;
            // (chart.value as any).update(
            //   { ...themeOptions[ca.value.theme], ...newValue },
            //   props.redrawOnUpdate,
            //   (props as any).oneToOneOnUpdate,
