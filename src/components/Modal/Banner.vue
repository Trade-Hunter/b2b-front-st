<template>
  <div
    v-if="c !== -1"
    class="h-full z-50 place-items-center flex flex-row justify-center py-2 absolute text-2xl font-semibold italic text-center text-gray-900 dark:text-white"
  >
    <div
      class="flex bg-white dark:bg-dark-1 w-full md:w-2/3 md:h-64 rounded-lg border border-3 border-sky-400 transition transform hover:-translate-y-1 hover:shadow-sky-500/90 shadow-lg shadow-sky-500/70 ..."
    >
      <span
        class="animate-pulse absolute inline-flex h-full w-full rounded-md shadow-2xl shadow-sky-500/70 ... z-0"
      ></span>
      <div class="flex flex-col place-items-center self-center center">
        ...
        <div class="flex flex-col md:flex-row w-full">
          ...
          <div class="md:pt-4 pr-4 md:w-1/2 grow">
            ...
            <span
              class="hidden md:block relative rounded-md py-2 mb-2 md:whitespace-pre"
              >Quer ter acesso a todas as funcionalidades da plataforma<br />
              <button
            ...
            >
            <span
              class="block md:hidden relative rounded-md py-2 mb-2 md:whitespace-pre text-sm"
              >Quer ter acesso a todas as funcionalidades da plataforma
              <button
                class="bg-green-500 rounded-md px-2 font-semibold italic text-white"
              >
                SEM PAGAR NADA</button
              ><br />
              e ainda ganhar ferramentas exclusivas?</span
            >
            <div class="relative mb-2 md:mt-6 self-center z-0">
              <button
                @click="click"
      ...
      <div
        class="flex justify-end absolute z-0 p-1 self-start text-xs mb-1 w-full"
      >
        <span v-if="c > 0" class="mt-2 self-end text-xs mb-1">
          Aguarde {{ c - 1 }}s. para fechar
        </span>
        <button
          v-if="c == 0"
          @click="c--"
          class="mb-1 self-end text-xs ml-2 text-white bg-red-500 px-2 py-1 rounded-md"
        >
          ✖
        </button>
      </div>
