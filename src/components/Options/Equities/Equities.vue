<template>
  <HxcMenu>
    ...
    <div class="flex flex-col no-scrollbar overflow-scroll min-w-full h-full pl-2 pt-1">
      <div class="flex">
        <h2 class="mb-2 text-xl font-extrabold text-gray-900 dark:text-gray-50 font-mono">Realtime Ativo</h2>
        <div
      ...
      columns: [
        {
          ...
          custom_format: function (obj, value, colIndex, rowIndex) {
            return `<a href="/options/equities/${value}" class="font-semibold text-blue-500">${value}</a>`;
          },
        },
        { name: "Hora", value: "nexxusReceivedAt", format: "time" },
        { name: "Negócios", value: "neg", total: { calcMode: "sum", format: "int" }, format: "int" },
        { name: "Negócios Projetados", value: "npr", total: { calcMode: "sum", format: "int" }, format: "int" },
        {
          name: "Negócios Projetados / Média",
          value: "nprmmneg",
          format: "float",
          custom_format: function (obj, value, colIndex, rowIndex) {
            if (value >= 2) return `<p class="font-semibold text-green-500">${value?.toFixed(2)}</p>`;
            return value?.toFixed(2);
          },
        },
        { name: "Quantidade", value: "qtd", format: "int" },
        { name: "Quantidade Projetada", value: "qpr", format: "int" },
        {
          name: "Quantidade Projetada / Média",
          value: "qprmmqtt",
          format: "float",
          custom_format: function (obj, value, colIndex, rowIndex) {
            if (value >= 2) return `<p class="font-semibold text-green-500">${value?.toFixed(2)}</p>`;
        ...
        { name: "Volume Total", value: "vtt", total: { calcMode: "sum", format: "int" }, format: "int" },
        { name: "Volume Projetado", value: "vpr", total: { calcMode: "sum", format: "int" }, format: "int" },
        {
          name: "Volume Projetado / Média",
          value: "vprmmvtt",
          format: "float",
          custom_format: function (obj, value, colIndex, rowIndex) {
            if (value >= 2) return `<p class="font-semibold text-green-500">${value?.toFixed(2)}</p>`;
        ...
        },
        {
          name: "Prêmio Médio",
          value: "premioMedio",
          desc: "Média de todos os premios negociados",
          total: { calcMode: "mean", format: "float" },
          format: "float",
        },
        {
          name: "Prêmio Médio Ult",
          value: "premioMedioUlt",
          desc: "Premio médio / preço do ativo objeto",
    ...
    createWSClient() {
      return new Promise((resolve, reject) => {
        ...
        this.connection.onopen = (event) => {
          this.connection.send(`C=S;I=options;O=3212`);
          this.$notify({
            type: "success",
